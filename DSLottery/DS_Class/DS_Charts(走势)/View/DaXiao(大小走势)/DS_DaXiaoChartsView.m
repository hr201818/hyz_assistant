...
@interface DS_DaXiaoChartsView () <UIScrollViewDelegate>
...
@implementation DS_DaXiaoChartsView
...
- (void)layoutView {
    ...
    //期号
    DS_LotteryPeriodsView *  issueView = [[DS_LotteryPeriodsView alloc]initWithFrame:CGRectMake(0, 0, 100, (self.periodsNumber.count + 4) * cellHeightDraw)];
    ...
    openCodeView.lotteryId = self.LotteryID;
    ...
    for (int i = 1; i<=codeArray.count; i++) {
        DS_JiOuDaxiaoTopView * JiouDaxiaoTopView = [[DS_JiOuDaxiaoTopView alloc]initWithFrame:CGRectMake(left, 0, 90, 40) titleName:[NSString stringWithFormat:@"第%d位",i] leftName:@"大" rightName:@"小"];
        ...
        DS_LotteryDaxiaoPathView * JiouPathView = [[DS_LotteryDaxiaoPathView alloc]initWithFrame:CGRectMake(left, 0, JiouDaxiaoTopView.width,issueView.height)];
        JiouPathView.codeList = oneCode;
        JiouPathView.lottery_ID = self.LotteryID;
...
- (UIScrollView *)tagScrollView {
    if (!_tagScrollView) {
        _tagScrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(100, 0, self.width - 100, 40)];
...
- (UIScrollView *)qishuScrollView {
    if (!_qishuScrollView) {
        _qishuScrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 40, 100, self.height - 40)];
...
- (UIScrollView *)codeScrollView {
    if (!_codeScrollView) {
        _codeScrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(100, 40, self.width - 100, self.height - 40)];
