...
@interface DSDrawingDaxiaoView()<UIScrollViewDelegate>
...
@implementation DSDrawingDaxiaoView
...
-(void)layoutView{
    //标签
    self.tagScrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(100, 0, self.width - 100, 40)];
    ...
    //期数
    self.qishuScrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 40, 100, self.height - 40)];
    ...
    //开奖号码
    self.codeScrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(100, 40, self.width - 100, self.height - 40)];
    ...
    //期号
    DSLotteryIssueView *  issueView = [[DSLotteryIssueView alloc]initWithFrame:CGRectMake(0, 0, 100, (self.periodsNumber.count + 4) * cellHeightDraw)];
    ...
    for (int i = 1; i<=codeArray.count; i++) {
        DSJiouDaxiaoTopView * JiouDaxiaoTopView = [[DSJiouDaxiaoTopView alloc]initWithFrame:CGRectMake(left, 0, 90, 40) titleName:[NSString stringWithFormat:@"第%d位",i] leftName:@"大" rightName:@"小"];
        ...
        DSLotteryDaxiaoPathView * JiouPathView = [[DSLotteryDaxiaoPathView alloc]initWithFrame:CGRectMake(left, 0, JiouDaxiaoTopView.width,issueView.height)];
