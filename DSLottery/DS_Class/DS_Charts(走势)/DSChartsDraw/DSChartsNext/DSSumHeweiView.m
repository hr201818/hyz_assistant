...
-(void)drawRect:(CGRect)rect{
    ...
    for (int i=0; i<listCount+4; i++) {
        ...
        if(i<listCount){
            ...
            [[[NSString stringWithFormat:@"%ld",all]substringFromIndex:[NSString stringWithFormat:@"%ld",all].length - 1] drawInRect:CGRectMake(0,i*cellHeight+ (cellHeight - 18)/2,self.width, cellHeight) withAttributes:dict];
        }
    }
    //获得处理的上下文
